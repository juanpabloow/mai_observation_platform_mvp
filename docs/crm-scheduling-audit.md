...
**E12 — Token model.** **No per-capability scoping.** `handoff_tokens` has no scope/capability column; scheduling auth calls the **same** `authenticateHandoffRequest` as the handoff API. So **a token minted "for handoff" can create/cancel/reschedule appointments.** The only gate is per-request: `X-Workflow-Ref` must resolve to a workflow under that token's connection whose client is non-default with `scheduling` enabled. There is no handoff-only vs scheduling-only token.
