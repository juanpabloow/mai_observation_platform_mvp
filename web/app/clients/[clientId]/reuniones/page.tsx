...
import { DETAILED_IDS, listMeetings, MEETINGS_SUMMARY, type MeetingFacet } from "@/lib/meetingsData";
...
export default async function ClientMeetingsPage({
  ...
  const { client } = await requireClientModulePage(clientId, "meetings");
  ...
  const all = await listMeetings({ facet, search, sort });
    ...
    <main className="flex min-h-0 w-full flex-1 flex-col gap-[var(--content-pad)]">
      ...
      <PageShell grow={false} clip={false}>
        <div className="flex flex-wrap items-center gap-2.5 px-3 py-2.5">
          <PageHeading title="Reuniones" count={MEETINGS_SUMMARY.total} />
          <MeetingsSearch />
      ...
      <PageShell clip={false}>
        ...
        <div className="flex flex-wrap items-center gap-x-3 gap-y-2 border-b border-line-row px-3 py-2">
          <FacetPills
            ...
            items={[
              { key: "all", label: "Todas", count: MEETINGS_SUMMARY.total, href: hrefWith({ facet: undefined, page: undefined }), active: facet === "all" },
              { key: "done", label: "Completadas", count: MEETINGS_SUMMARY.done, href: hrefWith({ facet: "done", page: undefined }), active: facet === "done" },
              {
                key: "processing",
                label: "En proceso",
                count: MEETINGS_SUMMARY.processing,
              ...
              },
              {
                ...
                count: MEETINGS_SUMMARY.attention,
          ...
          <span className="ml-auto flex flex-wrap items-center gap-x-3 gap-y-2">
            ...
            <span className="flex items-center gap-2 text-[0.78125rem] text-muted">
              <span>
                <span className="font-medium text-foreground u-mono">{MEETINGS_SUMMARY.hoursTranscribed}</span> h transcritas
              </span>
              <span aria-hidden className="text-faintest">
                ·
              </span>
              <span>
                <span className="font-medium text-foreground u-mono">{MEETINGS_SUMMARY.openTasks}</span> tareas abiertas
              </span>
            </span>
        ...
        {meetings.length === 0 ? (
          ...
          <>
            <div className="min-h-0 flex-1 overflow-auto">
              <MeetingsTable meetings={meetings} detailedIds={DETAILED_IDS} hrefFor={(id) => `${base}/${id}`} sort={sort} />
            </div>
            <div className="flex shrink-0 flex-wrap items-center gap-3 border-t border-line-row px-4 py-2.5">
              <span className="text-[0.71875rem] text-faint">
                Mostrando {firstShown}–{lastShown} de {MEETINGS_SUMMARY.total} reuniones
              </span>
