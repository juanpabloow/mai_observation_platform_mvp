...
async function loadStaffTab(
  ...
  const [staff, services, todayAppts, windowAppts, exceptions] = await Promise.all([
    listStaff(tenantId, { siteId: site.id, clientId, includeInactive: false }),
  ...
  // One grouped query instead of one per barber — the roster is the only place that
  // needs every barber's services at once.
  const serviceIds = await listStaffServiceIds(
    tenantId,
    staff.map((s) => s.id),
  );
