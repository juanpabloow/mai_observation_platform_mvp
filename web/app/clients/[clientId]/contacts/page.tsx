...
export default async function ClientContactsPage({
  ...
  return (
    ...
    <main className="flex min-h-0 w-full flex-1 flex-col gap-[var(--content-pad)]">
      ...
      {/* THE TOOLBAR (design frame 20g): title + count, search, the filter/sort/columns
          ...
          row it used to carry were folded into here to match the design; stage/owner
          filtering now lives in the Filtrar menu. `clip={false}` so the Filtrar / Orden /
          Columnas popovers are not clipped by the card's `overflow-hidden`. */}
